...
TRANSLATIONS = {
    ...
    'The YARPP Yet Another Related Posts Plugin plugin for WordPress is vulnerable to unauthorized access due to a missing capability check in the ~/includes/yarpp_pro_set_display_types.php file in all versions up to, and including, 5.30.10. This makes it possible for unauthenticated attackers to set display types.': "Wtyczka WordPress o nazwie  YARPP Yet Another Related Posts Plugin w wersji do 5.30.10 włącznie umożliwia atakującemu zdalne wykonanie kodu." + RCE_EFFECT_DESCRIPTION + WORDPRESS_UPDATE_HINT,
     'RCE in Jolokia < 1.7.1 using AccesLogValve' : "Narzędzie Jolokia w wersji poniżej 1.7.1 zawiera podatność umożliwiającą zdalne wykonanie kodu." + RCE_EFFECT_DESCRIPTION + UPDATE_HINT,
