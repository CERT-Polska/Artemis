...
def test_subfinder_reports_failure_when_both_runs_fail() -> None:
    ...
    assert SubdomainEnumeration.get_subdomains_from_subfinder(stub, "example.com") is None  # type: ignore


def test_subfinder_keeps_the_results_of_the_run_that_worked() -> None:
    stub = stub_with_tool_results([None, {"a.example.com"}])
    assert SubdomainEnumeration.get_subdomains_from_subfinder(stub, "example.com") == {"a.example.com"}  # type: ignore


def test_subfinder_returns_an_empty_set_when_both_runs_find_nothing() -> None:
    stub = stub_with_tool_results([set(), set()])
    assert SubdomainEnumeration.get_subdomains_from_subfinder(stub, "example.com") == set()  # type: ignore
