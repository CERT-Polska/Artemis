...
TRANSLATIONS = {
    ...
    + REFLECTED_XSS_DESCRIPTION
    + UPDATE_HINT,
    "webp_server_go 0.4.0 contains a path traversal caused by insufficient sanitization in file handling, letting attackers read arbitrary files on the server, exploit requires attacker to send crafted requests.": "webp_server_go w wersji 0.4.0 i potencjalnie wcześniejszych zawiera podatność Path Traversal o identyfikatorze CVE-2021-46104, umożliwiającą atakującemu odczyt dowolnych plików z serwera poprzez wysłanie odpowiednio spreparowanego żądania." + UPDATE_HINT,
    "Detected exposed Batflat CMS SQLite database files that may contain sensitive information including admin credentials, user data, site configuration, and content. Batflat stores its database in the /inc/data/ directory by default.": "Wykryto publicznie dostępne pliki bazy danych SQLite systemu Batflat CMS, które mogą zawierać poufne informacje, takie jak dane logowania administratora, dane użytkowników, konfiguracja strony i treści." + DATA_HIDE_HINT,
