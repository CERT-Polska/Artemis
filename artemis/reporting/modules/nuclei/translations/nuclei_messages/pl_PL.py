...
TRANSLATIONS = {
    ...
    "Flowise is a drag & drop user interface to build a customized large language model flow. Prior to version 3.0.13, the NVIDIA NIM router (/api/v1/nvidia-nim/*) is whitelisted in the global authentication middleware, allowing unauthenticated access to privileged container management and token generation endpoints.": "Wersje Flowise przed 3.0.13 zawierają podatność umożliwiającą atakującemu uzyskanie nieautoryzowanego dostępu do uprzywilejowanych końcówek zarządzania kontenerami i generowania tokenów."
    + UPDATE_HINT,
    "Barracuda Email Security Gateway (ESG) appliances are vulnerable to a remote code execution through a specially crafted Microsoft Excel attachment.\nIn some cases it's not possible to extract the version of the Barracuda ESG appliance, therefore this template might miss some vulnerable instances.": "Wykryto urządzenie Barracuda Email Security Gateway (ESG) zawierające podatność umożliwiającą zdalne wykonanie kodu poprzez specjalnie spreparowany załącznik Microsoft Excel.",
    "A security issue was discovered in Kubernetes where under certain conditions, an unauthenticated attacker with access to the pod network can achieve arbitrary code execution in the context of the ingress-nginx controller. This can lead to disclosure of Secrets accessible to the controller. (Note that in the default installation, the controller can access all Secrets cluster-wide.)": "Wykryto podatność w systemie Kubernetes umożliwiającą atakującemu zdalne wykonanie kodu w kontekście kontrolera ingress-nginx, co może prowadzić do ujawnienia sekretów dostępnych dla tego kontrolera.",
