...
TRANSLATIONS = {
    ...
    + RCE_EFFECT_DESCRIPTION
    + UPDATE_HINT,
    "Oracle GlassFish Server Open Source Edition 4.1 is vulnerable to both authenticated and unauthenticated local file inclusion vulnerabilities that can be exploited by issuing specially crafted HTTP GET requests.": "Oracle GlassFish Server Open Source Edition w wersji 4.1 i potencjalnie wcześniejszych umożliwia zarówno uwierzytelnionemu, jak i nieuwierzytelnionemu atakującemu odczyt dowolnych plików z dysku."
    + UPDATE_HINT,
