...
def upgrade():
    ...
    sa.Column('fulltext', artemis.db.TSVector(), sa.Computed("to_tsvector('english', COALESCE(tag, '') || ' ' || COALESCE(target, ''))", persisted=True), nullable=True),
    ...
    op.create_table('report_generation_task',
    ...
    sa.Column('fulltext', artemis.db.TSVector(), sa.Computed("to_tsvector('english', COALESCE(status, '') || ' ' || COALESCE(tag, '') || ' ' || COALESCE(target_string, '') || ' ' || COALESCE(headers_string, '') || ' ' || COALESCE(status_reason, ''))", persisted=True), nullable=True),
    sa.PrimaryKeyConstraint('id')
    )
