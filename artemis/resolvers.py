    ...
    for entry in answer:
        ...
        if entry["type"] == 5:
            ...
            for subentry in answer:
                if (
                    subentry["name"] == entry["data"].rstrip(".")
                    and subentry["type"] == 1
                ):
...
def ip_lookup(domain: str) -> Set[str]:
    ...
    response = requests.get(
        f"{DOH_SERVER}?name={domain}&type=A", headers={"accept": "application/dns-json"}
    )
    if not response.ok:
        raise IPResolutionException(
            f"DOH server invalid response ({response.status_code})"
        )
