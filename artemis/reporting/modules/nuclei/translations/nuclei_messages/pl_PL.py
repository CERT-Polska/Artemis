...
TRANSLATIONS = {
    ...
    "A security issue was discovered in  ingress-nginx https-//github.com/kubernetes/ingress-nginx  where the `mirror-target` and `mirror-host` Ingress annotations can be used to inject arbitrary configuration into nginx. This can lead to arbitrary code execution in the context of the ingress-nginx controller, and disclosure of Secrets accessible to the controller. (Note that in the default installation, the controller can access all Secrets cluster-wide.)": "Wykryto komponent ingress-nginx zawierający podatność CVE-2025-1098 umożliwiającą atakującemu zdalne wykonanie kodu." + RCE_EFFECT_DESCRIPTION + UPDATE_HINT,
    "appsetting.json file discloses the DB connection strings containing sensitive information.": "Wykryto plik appsetting.json zawierający dane dostępowe do bazy danych." + DATA_HIDE_HINT,
