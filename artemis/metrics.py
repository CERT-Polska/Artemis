...
from prometheus_client import REGISTRY, start_http_server, GC_COLLECTOR, PLATFORM_COLLECTOR, PROCESS_COLLECTOR
...
class ArtemisMetricsCollector:
    ...
    def collect(self):
        ...
        for karton_name in high_level_kartons:
            num_tasks_high_level_kartons += sum([backend.redis.llen(key) for key in backend.redis.keys(f"karton.queue.*:{karton_name}")])
