...
class MailDNSScanner(ArtemisBase):
    ...
    def run(self, current_task: Task) -> None:
        ...
        if result.spf_dmarc_scan_result and result.spf_dmarc_scan_result.spf:
            result.spf_dmarc_scan_result.spf.warnings = self._filter_warnings(result.spf_dmarc_scan_result.spf.warnings)
        ...
        if result.spf_dmarc_scan_result and result.spf_dmarc_scan_result.dmarc:
            result.spf_dmarc_scan_result.dmarc.warnings = self._filter_warnings(
                result.spf_dmarc_scan_result.dmarc.warnings
            )
        ...
        if result.spf_dmarc_scan_result and result.spf_dmarc_scan_result.ssl:
            status_reasons.extend(["Problem for server %s port %s: %s" % (item.mx, item.port, item.error) for item in result.spf_dmarc_scan_result.ssl.results if item.error])
