...
TRANSLATIONS = {
    ...
    + RCE_EFFECT_DESCRIPTION
    + UPDATE_HINT,
    "tagDiv Composer plugin versions before 4.2 for WordPress are vulnerable to unauthenticated stored XSS via the /wp-json/tdw/save_css endpoint. An attacker can inject malicious JavaScript code through the compiled_css parameter, which gets stored and executed when the CSS is loaded.": "Wtyczka WordPress o nazwie tagDiv Composer w wersji poniżej 4.2 zawiera podatność Unauthenticated XSS, umożliwiającą atakującemu wstrzyknięcie szkodliwego kodu do strony internetowej." + WORDPRESS_UPDATE_HINT,
    "TOTOLINK Realtek SDK based routers may allow an authenticated attacker to execute arbitrary OS commands via the sysCmd parameter to the boafrm/formSysCmd URI, even if the GUI (syscmd.htm) is not available. This allows for full control over the device\'s internals. This affects A3002RU through 2.0.0, A702R through 2.1.3, N301RT through 2.1.6, N302R through 3.4.0, N300RT through 3.4.0, N200RE through 4.0.0, N150RT through 3.4.0, and N100RE through 3.4.0.": "Routery oparte na oprogramowaniu TOTOLINK Realtek SDK: A3002RU w wersji do 2.0.0, A702R w wersji do 2.1.3, N301RT w wersji do 2.1.6, N302R w wersji do 3.4.0, N300RT w wersji do 3.4.0, N200RE w wersji do 4.0.0, N150RT w wersji do 3.4.0 i N100RE w wersji do 3.4.0 mogą umożliwić uwierzytelniotemu atakującemu wykonywać dowolne polecenia systemowe, nawet jak interfejs GUI nie jest dostępny. Umożliwia to pełną kontrolę nad urządzeniem.",
