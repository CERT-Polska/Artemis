...
class PostgresSqlInjectionDetectorTestCase(ArtemisModuleTestCase):
    ...
    def test_sql_injection_detector(self) -> None:
        task = Task(
            ...
            payload={"host": "test-apache-with-sql-injection-postgres.local", "port": 80},
        )
        self.run_task(task)
        (call,) = self.mock_db.save_task_result.call_args_list

        sqli_message = "http://test-apache-with-sql-injection-postgres.local/sql_injection.php?id='\""
        time_base_sqli_message = (
            "http://test-apache-with-sql-injection-postgres.local/sql_injection.php?id='||pg_sleep(5)||'"
            ": It appears that this URL is vulnerable to time-based SQL injection"
        )
        sqli_by_headers_message = (
            "http://test-apache-with-sql-injection-postgres.local/headers_vuln.php: "
    ...
    def test_is_url_with_parameters(self) -> None:
        url_with_payload = "http://test-apache-with-sql-injection-postgres.local?id=3"
        current_url = "http://test-apache-with-sql-injection-postgres.local"
    ...
    def test_measure_request_time(self) -> None:
        current_url = "http://test-apache-with-sql-injection-postgres.local/sql_injection.php?id=1"
    ...
    def test_contains_error(self) -> None:
        ...
        url_with_payload = "http://test-apache-with-sql-injection-postgres.local/sql_injection.php?id='"
        url_to_headers_vuln = "http://test-apache-with-sql-injection-postgres.local/headers_vuln.php"
...
class MysqlSqlInjectionDetectorTestCase(ArtemisModuleTestCase):
    ...
    def test_sql_injection_detector(self) -> None:
        task = Task(
            {"type": TaskType.SERVICE.value, "service": Service.HTTP.value},
            payload={"host": "test-apache-with-sql-injection-mysql.local", "port": 80},
        )

        self.run_task(task)
        (call,) = self.mock_db.save_task_result.call_args_list

        sqli_message = "http://test-apache-with-sql-injection-mysql.local/sql_injection.php?id='\""
        time_base_sqli_message = (
    ...
    def test_is_url_with_parameters(self) -> None:
        ...
        url_with_payload = "http://test-apache-with-sql-injection-mysql.local?id=3"

        self.assertTrue(self.karton_class.is_url_with_parameters(url_with_payload))
        self.assertFalse(self.karton_class.is_url_with_parameters(current_url))

    def test_measure_request_time(self) -> None:
        current_url = "http://test-apache-with-sql-injection-mysql.local/sql_injection.php?id=5"
    ...
    def test_contains_error(self) -> None:
        ...
        url_with_payload = "http://test-apache-with-sql-injection-mysql.local/sql_injection.php?id='"
        url_to_headers_vuln = "http://test-apache-with-sql-injection-mysql.local/headers_vuln.php"
