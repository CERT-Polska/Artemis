...
TRANSLATIONS = {
    ...
    "The Django settings.py file containing a secret key was discovered. An attacker may use the secret key to bypass many security mechanisms and potentially obtain other sensitive configuration information (such as database password) from the settings file.": "Wykryto plik settings.py zawierający konfigurację systemu Django. Atakujący może wykorzystać te dane aby ominąć mechanizmy bezpieczeństwa, lub (jeśli znajduje się tam np. hasło do bazy danych) uzyskać nieuprawniony dostęp do systemu." + DATA_HIDE_HINT,
