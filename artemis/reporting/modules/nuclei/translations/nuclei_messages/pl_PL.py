...
TRANSLATIONS = {
    ...
    + UPDATE_HINT,
    "WordPress is affected by an unauthenticated blind SSRF in the pingback feature. Because of a TOCTOU race condition between the validation checks and the HTTP request, attackers can reach internal hosts that are explicitly forbidden.": "Wykryto system WordPress zawierający podatność typu blind SSRF (identyfikator CVE-2022-3590) niewymagającą uwierzytelnienia. Aakujący mogą wysyłać żądania HTTP do hostów w sieci wewnętrznej (bez możliwości odczytu odpowiedzi)."
    + UPDATE_HINT,
