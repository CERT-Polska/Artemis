...
class JoomlaScanner(ArtemisBase):
    ...
    def _is_version_old(self, version: str, age_threshold_days: int = 180) -> bool:
        cache_key = "versions"
    ...
    def run(self, current_task: Task) -> None:
        ...
        if "registration.register" in response.text:
            found_problems.append(
                f"Joomla registration is enabled in {registration_url}"
            )
        ...
        if match := re.search(
            "<version>([0-9]+\\.[0-9]+\\.[0-9]+)</version>", response.text
        ):
            ...
            if gh_api_response.json()[
                "tag_name"
            ] != joomla_version and self._is_version_old(joomla_version):
