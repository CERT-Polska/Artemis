...
TRANSLATIONS = {
    ...
    + REFLECTED_XSS_DESCRIPTION
    + UPDATE_HINT,
    "Pterodactyl is a free, open-source game server management panel. Using the /locales/locale.json with the locale and namespace query parameters, a malicious actor is able to execute arbitrary code without being authenticated.": "Wykryto system Pterodactyl w wersji zawierającej podatność umożliwiającą nieuprawnione wykonanie kodu." + RCE_EFFECT_DESCRIPTION,
    ...
    "Responsive FileManager before version 9.13.4 is vulnerable to local file inclusion via filemanager/ajax_calls.php because it uses external input to construct a pathname that should be within a restricted directory, aka local file inclusion.": "Responsive FileManager w wersji poniżej 9.13.4 zawiera podatność Local File Inclusion, umożliwiającą atakującemu odczyt całej zawartości bazy danych.",
