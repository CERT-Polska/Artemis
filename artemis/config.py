...
class Config:
    ...
    ALLOW_SCANNING_PUBLIC_SUFFIXES = decouple.config(
        "ALLOW_SCANNING_PUBLIC_SUFFIXES", default=False, cast=bool
    )

    # additional domains that will be treated as public suffixes (even though they're not on the default Public Suffix List)
    ADDITIONAL_PUBLIC_SUFFIXES = decouple.config(
        "ADDITIONAL_PUBLIC_SUFFIXES", default="", cast=decouple.Csv(str)
    )
    ...
    VERIFY_REVDNS_IN_SCOPE = decouple.config(
        "VERIFY_REVDNS_IN_SCOPE", default=True, cast=bool
    )

    # What paths to skip in the robots and directory_index modules
    NOT_INTERESTING_PATHS = decouple.config(
        "NOT_INTERESTING_PATHS", default="/icon/,/icons/", cast=decouple.Csv(str)
    )

    # default request timeout (for all protocols)
    REQUEST_TIMEOUT_SECONDS = decouple.config(
        "REQUEST_TIMEOUT_SECONDS", default=5, cast=int
    )
    ...
    SECONDS_PER_REQUEST_FOR_ONE_IP = decouple.config(
        "SECONDS_PER_REQUEST_FOR_ONE_IP", default=0, cast=int
    )

    # E.g. when set to 100, Artemis will strive to send no more than 100 port scanning packets per seconds to any host.
    SCANNING_PACKETS_PER_SECOND_PER_IP = decouple.config(
        "SCANNING_PACKETS_PER_SECOND_PER_IP", default=100, cast=int
    )

    # Whether Artemis should strive to make at most one module scan a target at a given time
    LOCK_SCANNED_TARGETS = decouple.config(
        "LOCK_SCANNED_TARGETS", default=False, cast=bool
    )
    ...
    LOCK_SLEEP_MIN_SECONDS = decouple.config(
        "LOCK_SLEEP_MIN_SECONDS", default=0.1, cast=float
    )
    LOCK_SLEEP_MAX_SECONDS = decouple.config(
        "LOCK_SLEEP_MAX_SECONDS", default=0.5, cast=float
    )
    ...
    SCAN_DESTINATION_LOCK_MAX_TRIES = decouple.config(
        "SCAN_DESTINATION_LOCK_MAX_TRIES", default=2, cast=int
    )
    ...
    DEFAULT_LOCK_EXPIRY_SECONDS = decouple.config(
        "DEFAULT_LOCK_EXPIRY_SECONDS", default=3600, cast=int
    )

    # In order not to overload the DB and bandwidth, this determines how long
    # the downloaded content would be (in bytes).
    CONTENT_PREFIX_SIZE = decouple.config(
        "CONTENT_PREFIX_SIZE", default=10240, cast=int
    )
    ...
    BRUTER_NUM_TOP_PATHS_TO_USE = decouple.config(
        "BRUTER_NUM_TOP_PATHS_TO_USE", default=800, cast=int
    )
    BRUTER_NUM_RANDOM_PATHS_TO_USE = decouple.config(
        "BRUTER_NUM_RANDOM_PATHS_TO_USE", default=600, cast=int
    )
    ...
    BRUTER_FOLLOW_REDIRECTS = decouple.config(
        "BRUTER_FOLLOW_REDIRECTS", default=True, cast=bool
    )

    # custom port list to scan in CSV form (replaces default list)
    CUSTOM_PORT_SCANNER_PORTS = decouple.config(
        "CUSTOM_PORT_SCANNER_PORTS", default="", cast=decouple.Csv(int)
    )
    ...
    NUCLEI_CHECK_TEMPLATE_LIST = decouple.config(
        "NUCLEI_CHECK_TEMPLATE_LIST", default=True, cast=bool
    )
