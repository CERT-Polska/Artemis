...
TRANSLATIONS = {
    ...
    "A response-header CRLF injection vulnerability in the Proxmox Virtual Environment (PVE) and Proxmox Mail Gateway (PMG) web interface allows a remote attacker to set cookies for a victim's browser that are longer than the server expects, causing a client-side DoS. This affects Chromium-based browsers because they allow injection of response headers with %0d. This is fixed in pve-http-server 4.1-3.": "Wykryto oprogramowanie Proxmox Virtual Environment (PVE) lub Proxmox Mail Gateway (PMG) zawierające podatność umożliwiającą uniemożliwienie ofierze dostępu do systemu.",
    "Searches for sensitive directories present in the sym404.": "Wykryto wtyczkę WordPress o nazwie sym404 w wersji umożliwiającej atakującemu pobranie dowolnych plików z serwera.",
